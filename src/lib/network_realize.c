...
int guiseClientRealizeUpdate(GuiseClientRealize* self, MonotonicTimeMs now)
{
    int clientUpdateErr = guiseClientUpdate(&self->client, now);
    if (clientUpdateErr < 0) {
        return clientUpdateErr;
    }
